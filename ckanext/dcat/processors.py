...
class RDFSerializer(RDFProcessor):
    ...
    def graph_from_dataset(self, dataset_dict):
        ...
        holder_identifier = (dataset_dict.get('holder_identifier') or '')
        if holder_identifier and 'm_lps' in holder_identifier:
           dataset_ref1=dataset_ref1.replace(PREF_LANDING,"http://dati.lavoro.gov.it/")
        if holder_identifier and 'r_emiro' in holder_identifier:
...
#        dataset_dict['extras'].append({'key': 'source_catalog_homepage', 'value': source_uri})
        ...
        if (root_catalog_ref, DCT.hasPart, catalog_ref) not in g:
            ...
            for item in sources:
                ...
                if value:
                 ...
                 if key == 'source_catalog_homepage' and not value.endswith("/#"):
                   value = value + '/'
                   if 'uni_ba' in (dataset_dict.get('holder_identifier') or ''):
                    if key == 'source_catalog_homepage':
            ...
            if (dataset_dict.get('holder_identifier') or ''):
             identifier=(dataset_dict.get('holder_identifier') or '')
            ...
            if _pub:
                ...
                for src_key, _type, predicate, required in publisher_sources:
                    ...
                    if src_key == 'type':
                       if (dataset_dict.get('holder_identifier') or ''):
                          if 'r_' in (dataset_dict.get('holder_identifier') or '') or 'p_' in (dataset_dict.get('holder_identifier') or '') :
                           val="http://purl.org/adms/publishertype/RegionalAuthority"
                          if 'm_' in (dataset_dict.get('holder_identifier') or ''):
                           val="http://purl.org/adms/publishertype/NationalAuthority"
                          if 'c_' in (dataset_dict.get('holder_identifier') or ''):
                           val="http://purl.org/adms/publishertype/LocalAuthority"
                          if 'inail' in (dataset_dict.get('holder_identifier') or '') or 'inps' in (dataset_dict.get('holder_identifier') or '') or 'agid' in (dataset_dict.get('holder_identifier') or ''):
                           val="http://purl.org/adms/publishertype/NationalAuthority"
                          if 'anac' in (dataset_dict.get('holder_identifier') or '') or 'ispra' in (dataset_dict.get('holder_identifier') or '') or 'pcm' in (dataset_dict.get('holder_identifier') or ''):
                           val="http://purl.org/adms/publishertype/NationalAuthority"                            
                    if src_key == 'url':
